...
x <- rnbinom(n, size = rho0, prob = 1 / (1 + r * mu0 / rho0)) # number of incidents that a nurse experiences during r shifts
x_bar <- mean(x)
x2_bar <- mean(x ** 2)
...
for(t in 1:T){
  ...
  for(j in 1:300){
    ...
    prob[j] <- (dgamma(j * 0.005, shape = 1, rate = 1, log = FALSE) / (gamma(j * 0.005) ** n)) * exp(-(j * 0.005 / mu[t + 1]) * sum(lambda) + n * j * 0.005 * log(j * 0.005 / mu[t + 1]) + (j * 0.005 - 1) * sum(log(lambda)))
  }
  c <- sum(prob)
  rho[t + 1] <- sample((1:300) * 0.005, size = 1, replace = TRUE, prob = prob / c)
  
  # probability experience at least 14 incidents
  N[t + 1] <- 1 - pnbinom(13, size = rho[t + 1], prob = 1 / (1 + r * mu[t + 1] / rho[t + 1]), log = FALSE)
}
...
write_xlsx(dg,'~/Documents/Mathematical Sciences/Research Thesis/R/Plot/Gibbs sampler/simulated_dataset_incidents_Gibbs.xlsx')
write_xlsx(df,'~/Documents/Mathematical Sciences/Research Thesis/R/Plot/Gibbs sampler/simulated_dataset_outcome_Gibbs.xlsx')

setwd('/Users/patricia/Documents/Mathematical Sciences/Research Thesis/R/Plot/Gibbs sampler')
